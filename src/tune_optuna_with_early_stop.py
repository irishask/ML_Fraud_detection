...
Quality/latency balance is controlled by the `quality` parameter:
    ...
    "med_high" — extended,    ~9h,    ~0.0005 AUC loss vs high
...
# Pass quality="min" | "med" | "med_high" | "high" to each tune_*() function.
#
...
def _get_profile(quality):
    ...
    Parameters
    ----------
    quality : str — "min" | "med" | "med_high" | "high"
...
def _make_progress_callback(pbar):
    """
    Return an Optuna callback that updates the given tqdm progress bar.

    Parameters
    ----------
    pbar : tqdm instance — the progress bar to update

    Returns
    -------
    callable — Optuna-compatible callback (study, trial) -> None
    """
...
def tune_lgbm(X_train, y_train, X_val, y_val,
    ...
    quality   : str          — "min" | "med" | "med_high" | "high" (default: "med")
